...
execute store result score #tmp2 fkbm.var run data get entity @e[type=minecraft:wither_skeleton,distance=..3,sort=nearest,limit=1] UUID[0]
execute store success score #tmp3 fkbm.var run execute if score #tmp1 fkbm.var = #tmp2 fkbm.var

# block arrow
execute if score #tmp3 fkbm.var matches 0 run data merge entity @s {Tags:["fkbm.wither_skeleton.blocked"],Motion:[0d,0d,0d],damage:0d}
