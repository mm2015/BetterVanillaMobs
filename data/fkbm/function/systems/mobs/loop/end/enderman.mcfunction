...
execute if entity @s[tag=fkbm.enderman.vortex,scores={fkbm.var=8..}] run function fkbm:systems/mobs/loop/skill/enderman/vortex
...
# lookbreak
execute if entity @s[tag=fkbm.enderman.lookbreak,scores={fkbm.cd1=..0},nbt=!{AngerTime:0}] if entity @p[distance=..4,gamemode=!creative,gamemode=!spectator] anchored eyes facing entity @p eyes rotated ~ 0 positioned ^ ^ ^.6 unless block ~ ~ ~ air if block ~ ~-1 ~ air if block ~ ~ ~ #fkbm:enderman_lookbreak if block ~ ~-2 ~ #fkbm:enderman_lookbreak run function fkbm:systems/mobs/loop/skill/enderman/lookbreak
